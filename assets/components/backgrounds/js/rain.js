...
(function () {
  ...
  function mount(root) {
    ...
    // O ruído é suave: acima de 1× a diferença é ≤ 1/255 e o custo de GPU dobra.
    function size(){const dpr=Math.min(devicePixelRatio||1,1);canvas.width=Math.max(1,Math.round(root.clientWidth*dpr));canvas.height=Math.max(1,Math.round(root.clientHeight*dpr));draw();}
